...
export type QueryAdapter = {
	find(collection: string, opts: { page?: number; limit?: number; sort?: string; where?: Record<string, unknown> }): Promise<{ docs: Record<string, unknown>[]; totalDocs: number; totalPages: number; page: number }>;
