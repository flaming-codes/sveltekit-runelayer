...
const isOwnerOrAdmin: AccessFn = ({ req, id }) => {
...
describe('Role-Based Access Control — Full Journey', () => {
	...
	beforeAll(async () => {
		...
		kit = createRunekit(defineConfig({
	...
	describe('Public Pages collection', () => {
		...
		it('admin can create a page', async () => {
			const page = await create(ctx(PublicPages, adminReq), { title: 'About Us', content: 'We are a tech company.' });
			pageId = page.id as string;
			expect(page.title).toBe('About Us');
		});

		it('editor can create a page', async () => {
			const page = await create(ctx(PublicPages, editorReq), { title: 'Contact', content: 'Reach us at...' });
			expect(page.title).toBe('Contact');
		});

		it('regular user can create a page (isLoggedIn)', async () => {
			const page = await create(ctx(PublicPages, userReq), { title: 'My Page', content: 'User content' });
		...
		it('editor can update a page', async () => {
			const updated = await update(ctx(PublicPages, editorReq), pageId, { content: 'Updated content' });
		...
		it('editor CANNOT delete a page (requires admin)', async () => {
			await expect(
				remove(ctx(PublicPages, editorReq), pageId)
			).rejects.toThrow('Forbidden');
		});
	...
	describe('Internal Notes collection', () => {
		...
		it('authenticated user can create a note', async () => {
			const note = await create(ctx(InternalNotes, userReq), { title: 'Team standup', body: 'Discussed roadmap.', priority: 'high' });
		...
		it('unauthenticated user CANNOT read notes', async () => {
			await expect(
				find(ctx(InternalNotes, publicReq))
			).rejects.toThrow('Forbidden');
		});
		...
		it('regular user CANNOT delete a note', async () => {
			await expect(
				remove(ctx(InternalNotes, userReq), noteId)
			).rejects.toThrow('Forbidden');
		});
	...
	describe('Secret Docs collection (admin-only)', () => {
		...
		it('admin can create secret docs', async () => {
			const doc = await create(ctx(SecretDocs, adminReq), { title: 'Secret Plan', classification: 'top-secret' });
			expect(doc.title).toBe('Secret Plan');
		});

		it('editor CANNOT read secret docs', async () => {
			await expect(
				find(ctx(SecretDocs, editorReq))
			).rejects.toThrow('Forbidden');
		});
	...
	describe('Server-side access without request', () => {
		it('public collection (no access fn) works without request', async () => {
			...
			const openCollection = defineCollection({
				slug: 'open',
		...
		it('collections with access fns deny when no request is provided', async () => {
			await expect(
				create(ctx(SecretDocs), { title: 'Server-side attempt' })
			).rejects.toThrow('Forbidden');
		});
	});
});
