...
describe('File Upload & Media Management — Full Journey', () => {
	...
	it('upload handler accepts valid file uploads', async () => {
		...
		const res = await handler({ request: new Request('http://localhost/upload', { method: 'POST', body: formData }) });
	...
	it('upload handler rejects oversized files', async () => {
		...
		const formData = new FormData();
		formData.append('file', new File(['this is definitely more than 10 bytes of content'], 'big.txt', { type: 'text/plain' }));
	...
	it('upload handler rejects disallowed MIME types', async () => {
		...
		const formData = new FormData();
		formData.append('file', new File(['exe-content'], 'virus.exe', { type: 'application/x-msdownload' }));
	...
	it('handles batch upload of multiple files', async () => {
		...
		for (let i = 0; i < 10; i++) {
			const result = await storage.upload(
				...
				{ filename: `batch-${i}.txt`, mimeType: 'text/plain', folder: 'batch' },
			);
			results.push(result);
		}
