...
import { insertOne, findMany, findById, updateOne } from '../db/operations.js';
...
describe('Schema Evolution & Migration — Full Journey', () => {
	...
	it('creates initial schema with basic fields', () => {
		...
		insertOne(rdb.db, rdb.tables.products, { name: 'Doohickey C', price: 4.50 });
	...
	it('all data is intact after three schema evolutions', () => {
		const ProductsV3: CollectionConfig = defineCollection({
			...
			fields: [
				...
				{ name: 'price', ...number() },
				{ name: 'description', ...textarea() },
				{ name: 'category', ...select({ options: [{ label: 'Electronics', value: 'electronics' }, { label: 'Home', value: 'home' }, { label: 'Toys', value: 'toys' }] }) },
				{ name: 'inStock', ...checkbox() },
				{ name: 'sku', ...text() },
				{ name: 'weight', ...number() },
				{ name: 'metadata', ...json() },
