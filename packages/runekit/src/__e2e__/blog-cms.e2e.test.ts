...
describe('Blog CMS Platform — Full User Journey', () => {
	...
	it('posts table has version columns due to versions config', () => {
		const cols = kit.database.sqlite
	...
	let postReact: Record<string, unknown>;
	let postTailwind: Record<string, unknown>;
	...
	it('creates posts with relationships to authors and categories', async () => {
		...
		postReact = await create(postCtx, {
		...
		postTailwind = await create(postCtx, {
	...
	it('JSON metadata is stored and retrievable', async () => {
		...
		const meta = typeof post!.metadata === 'string'
			? JSON.parse(post!.metadata as string)
			: post!.metadata;
