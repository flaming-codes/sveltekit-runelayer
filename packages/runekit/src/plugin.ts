...
export interface RunekitInstance {
	...
	handle: (input: { event: any; resolve: (event: any) => Response | Promise<Response> }) => Promise<Response>;
}
...
export function createRunekit(config: RunekitConfig): RunekitInstance {
	...
	const handle = async ({ event, resolve }: { event: any; resolve: (event: any) => Response | Promise<Response> }) => {
