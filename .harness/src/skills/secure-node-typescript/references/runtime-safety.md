...
function convertImage(inputPath: string, outputPath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const process = spawn("convert", [safeInput, "-resize", "100x100", safeOutput], {
...
async function fetchMultipleUsers(ids: string[]): Promise<Map<string, User | null>> {
...
async function fetchWithTimeout<T>(operation: () => Promise<T>, timeoutMs: number): Promise<T> {
