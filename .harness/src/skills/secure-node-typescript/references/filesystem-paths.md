...
async function safeReadFile(userInput: string, baseDir: string): Promise<string> {
...
async function secureFileRead(userFilename: string, baseDir: string): Promise<Buffer> {
  // Validate extension
...
function validateDirectoryAccess(requestedPath: string, allowedKeys: DirectoryKey[]): string {
...
function isPathWithinBase(userPath: string, baseDir: string): boolean {
  ...
  return normalizedUser.startsWith(normalizedBase + path.sep) || normalizedUser === normalizedBase;
}
...
async function readSecureFile(filename: RelativePath, baseDir: DirPath): Promise<Buffer> {
...
class PathValidationError extends Error {
  constructor(
    ...
    public readonly reason: "traversal" | "invalid" | "not_found" | "permission",
  ) {
    super(message);
